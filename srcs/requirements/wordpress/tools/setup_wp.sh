...
if ! [ -f "/var/www/html/wp-config.php" ]; then
	...
	cd wp	
    wp core config --dbhost=${DB_HOST} --dbname=${MYSQL_WP_NAME} --dbuser=${DB_USER_NAME} --dbpass=${DB_USER_PASSWORD} --allow-root
    wp core install --url=${URL} --admin_user=${ADM_NAME} --admin_password=${ADM_PASSWORD} --admin_email=${ADM_EMAIL} --skip-email --allow-root
    wp user create --allow-root vaska vaskapupkin@mail.ru --role=author --user_pass=btwee
    mv * /var/www/html/
fi
